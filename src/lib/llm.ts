...
export async function callLLM(task: string, type: string, field: string) {
  try {
    const lessonPlanGuidelinePath = "src/data/inclusive_learning_design.pdf";
    const lessonPlanGuidelineRetriever = await getRetrieverFrom(lessonPlanGuidelinePath);
    const lessonPlanGuidelineDocs = await lessonPlanGuidelineRetriever.getRelevantDocuments(task);
